...
import{ BetterPagination, CategoryDelete} from "ui";


const SomeTable4 = ({ categories, router }: any) => {
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                  <tr>
                    {["Category", "ClinicId", "Patients", "Edit", "Delete"].map((item, index) => (
                  ...
                  {paginatedCategories.map((item: any, index: number) => {
                    return (
                      <tr key={index} className="border-b dark:border-gray-700">
                        ...
                        <td className="px-4 py-3">{item?.treatmentId}</td>
                        <td className="px-4 py-3">{item?.patients?.length}</td>

                        <td className="px-4 py-3 ">
                          <button
                            onClick={() => {
                              router.push(
                                `/admin/clinic/categories-editor/${item.name}`,
                                undefined,
                                { shallow: true },
                              );
                            }}
                        ...
                        <td className="px-4 py-3">
                          <button
                            type="button"
                            id={`deleteCategoryButton-${item.id}`}
                            data-te-toggle="modal"
                            data-te-target={`#category-delete-${item.id}`}
                            data-te-ripple-init
